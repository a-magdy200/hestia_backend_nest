...
describe('UserService - User Search', () => {
  ...
  describe('searchUsers', () => {
    it('should search users successfully', async () => {
      ...
      userRepository.searchUsers.mockResolvedValue(users as unknown as User[]);

      const result = await service.searchUsers({ email: 'test' }, mockRequestId);

      expect(result).toEqual({ users, total: 1, page: 1, limit: 10, totalPages: 1 });
    });
  });
});
